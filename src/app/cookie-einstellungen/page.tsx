    ...
    <>
      ...
      <section className="section bg-cream">
        <div className="container-content max-w-prose text-center">
          <span className="mx-auto inline-flex h-14 w-14 items-center justify-center rounded-full bg-burgundy/10 text-burgundy">
            <Cookie className="h-7 w-7" aria-hidden />
          </span>
          <p className="mt-6 text-fluid-base text-charcoal/75">{t('cookie.text')}</p>
          ...
          {consent && (
            <ul className="mx-auto mt-6 flex max-w-sm flex-col gap-2 text-left text-fluid-sm">
              {(['necessary', 'statistics', 'marketing', 'external'] as const).map((c) => (
                <li
                  key={c}
                  className="flex items-center justify-between rounded-lg border border-sand/50 bg-cream-soft px-4 py-2"
                >
                  <span className="text-charcoal/80">{t(`cookie.${c}` as never)}</span>
                  <span className={consent[c] ? 'font-medium text-emerald-600' : 'text-charcoal/50'}>
                    {consent[c] ? '✓' : '—'}
                  </span>
                </li>
              ))}
